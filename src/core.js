...
function shiftedDate(days, now = new Date()) {
...
export function daysAgoDate(daysBack, now = new Date()) {
  return shiftedDate(-Math.max(0, Number(daysBack) || 0), now);
}

export function daysAheadDate(daysAhead, now = new Date()) {
  return shiftedDate(Math.max(0, Number(daysAhead) || 0), now);
}
...
function point(row) {
  ...
  if (loc && typeof loc === 'object') {
    ...
    const lon = asNumber(loc.longitude);
    if (lat !== null && lon !== null) return { latitude: lat, longitude: lon };
  }
  return null;
}

export function signalSourceFamily(signal) {
  if (signal?.type === 'new_business_registration') return 'business_registry';
  if (String(signal?.type || '').startsWith('building_permit')) return 'building_permit';
  return String(signal?.type || 'other');
}
...
export function businessSignal(row) {
  ...
  return {
    ...
    sourceId: get(row, ['location_account']) || `${businessName || 'unknown'}:${address || ''}`,
...
export function permitSignal(row, stage = 'issued') {
  ...
  const eventDate = stage === 'submitted'
    ? get(row, ['submitted_date', 'submit_date', 'application_date'])
    : get(row, ['issue_date', 'issued_date']);
  ...
  return {
    ...
    eventDate, businessName: null, dbaName: null, address, city: 'LOS ANGELES', zipCode,
    vertical: classifyVertical(text), industry: useDescription, naics: null,
    valuation: asNumber(get(row, ['valuation', 'job_value', 'declared_value'])),
    permitNumber: get(row, ['permit_nbr', 'permit_number']), permitStatus: status,
    workDescription, useDescription, permitType, location: point(row), addressKey: addressKey(address, zipCode),
    isResidential: RESIDENTIAL_RE.test(text) && !COMMERCIAL_RE.test(text),
    isCommercialHint: COMMERCIAL_RE.test(text),
    sourceUrl: stage === 'submitted' ? 'https://data.lacity.org/d/gwh9-jnip' : 'https://data.lacity.org/d/pi9x-tg5x',
    raw: row,
  };
}
...
export function scoreOpportunity(signals, now = new Date()) {
  ...
  const sourceFamilies = new Set(signals.map(signalSourceFamily));
  ...
  const businessDates = signals
    .filter((s) => s.type === 'new_business_registration' && s.eventDate)
    .map((s) => new Date(s.eventDate))
    .filter((d) => !Number.isNaN(d.getTime()));
  const hasFutureBusinessStart = businessDates.some((d) => d > now);

  let score = recencyPoints(newestDate, now);
  const reasons = [];

  if (types.has('new_business_registration')) {
    score += 18;
    reasons.push(hasFutureBusinessStart ? 'future business start date on city registry' : 'recent business registry/start-date signal');
  }
  if (types.has('building_permit_submitted')) {
    score += 18;
    reasons.push('recent permit application');
  }
  if (types.has('building_permit_issued')) {
    score += 15;
    reasons.push('recent issued permit');
  }
  if (types.has('building_permit_submitted') && types.has('building_permit_issued')) {
    score += 5;
    reasons.push('permit progressed from submission to issuance');
  }
  if (sourceFamilies.size >= 2) {
    score += 28;
    reasons.push('independent business-registry and permit signals at the same address');
  }

  if (valuation >= 100000) {
    score += 10;
    reasons.push('high-value project');
  } else if (valuation >= 25000) {
    score += 6;
    reasons.push('meaningful project valuation');
  } else if (valuation >= 5000) {
    score += 3;
  }

  const vertical = signals.find((s) => s.vertical !== 'general_commercial')?.vertical || 'general_commercial';
  if (vertical !== 'general_commercial') {
    score += 4;
    reasons.push('commercial vertical identified');
  }

  score = Math.min(100, Math.round(score));
  const confidence = sourceFamilies.size >= 2 && score >= 70 ? 'high' : score >= 50 ? 'medium' : 'low';
  return {
    score,
    confidence,
    reasons,
    ...
    hasFutureBusinessStart,
  };
}

export function buildOpportunities(signals, {
  ...
  const filtered = signals.filter((signal) => signal.addressKey
    && (includeResidential || !signal.type.startsWith('building_permit') || !signal.isResidential)
    && (!zipSet.size || zipSet.has(parseZip5(signal.zipCode))));

  const groups = new Map();
  for (const signal of filtered) {
    if (!groups.has(signal.addressKey)) groups.set(signal.addressKey, []);
    groups.get(signal.addressKey).push(signal);
  }

  const results = [];
  for (const [key, group] of groups) {
    const scored = scoreOpportunity(group, now);
    const types = [...new Set(group.map((s) => s.type))];

    if (onlyMultiSignal && scored.sourceFamilies.length < 2) continue;
    if (scored.score < minScore || (verticalSet.size && !verticalSet.has(scored.vertical))) continue;

    const bestBusiness = group.find((s) => s.businessName || s.dbaName);
    const bestPermit = group.find((s) => s.type.startsWith('building_permit'));
    const primary = bestBusiness || bestPermit || group[0];
    const haystack = group.map((s) => [s.businessName, s.dbaName, s.industry, s.workDescription, s.permitType, s.useDescription, s.address, s.zipCode]
    ...
    results.push({
      opportunityId: `la:${key.replace(/[^A-Z0-9|]/gi, '').slice(0, 80)}`,
      opportunityScore: scored.score,
      confidence: scored.confidence,
      scoreReasons: scored.reasons,
      signalCount: group.length,
      signalTypes: types,
      sourceFamilies: scored.sourceFamilies,
      futureBusinessStart: scored.hasFutureBusinessStart,
      latestSignalDate: scored.newestDate,
      businessName: bestBusiness?.businessName || null,
      dbaName: bestBusiness?.dbaName || null,
      vertical: scored.vertical,
      vendorNeeds: vendorNeeds(scored.vertical),
      address: primary.address,
      city: primary.city || 'LOS ANGELES',
      zipCode: primary.zipCode,
      latitude: primary.location?.latitude ?? bestPermit?.location?.latitude ?? null,
      longitude: primary.location?.longitude ?? bestPermit?.location?.longitude ?? null,
      industry: bestBusiness?.industry || bestPermit?.industry || null,
      naics: bestBusiness?.naics || null,
      maxPermitValuation: scored.valuation || null,
      evidence,
  ...
  return results.sort((a, b) => b.opportunityScore - a.opportunityScore
    || String(b.latestSignalDate || '').localeCompare(String(a.latestSignalDate || '')));
}
